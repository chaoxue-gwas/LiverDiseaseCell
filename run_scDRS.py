...
class HeteroAnalysis:
    def __init__(self):
        ...
        self.out_dir= f'{PROJ_RESULT_DIR}/scdrs/analysis/hetero_cell'

    def ab_function(self,adata_ca1,eff_group,ref_group,out_prefix,top_n=200):
        sc.set_figure_params(figsize=[6, 6], dpi=150)
        adata_ca1 = adata_ca1[adata_ca1.obs['merged_groups'].isin([eff_group,ref_group])].copy()
        sc.tl.rank_genes_groups(adata_ca1, groupby='merged_groups', groups=[eff_group], reference=ref_group,
                                method='wilcoxon')
        result = adata_ca1.uns['rank_genes_groups']
        degs = pd.DataFrame({
            'gene': result['names'][eff_group],
            'pval_adj': result['pvals_adj'][eff_group],
            'score': result['scores'][eff_group],
        })
        df_sorted = degs.sort_values(by='score', ascending=False)
        ga_genes = df_sorted.head(top_n)['gene'].values
        gb_genes = df_sorted.tail(top_n)['gene'].values
        make_dir(os.path.dirname(out_prefix))
        gene_en=[[ga_genes,f'{out_prefix}.groupA.enrich.jpg','GroupA'],
                 [gb_genes,f'{out_prefix}.groupB.enrich.jpg','GroupB']]
        for ge in gene_en:
    ...
    def analysis_common(self,sc_data_key,plot_phenos,merge_cluster_arr:dict,select_celltype,out_prefix,plot_size=10):
        ...
        with open(scdrs_result_pyd, 'rb') as f:
            ...
            # assign scDRS score
            for trait in dict_score:
                adata_ca1.obs[trait] = dict_score[trait]["zscore"]
            sc.set_figure_params(figsize=[3, 3], dpi=150)
            sc.pl.umap(
                adata_ca1,
                color=plot_phenos,
            ...
            # function analysis of A and B
            self.ab_function(adata_ca1,'GroupA','GroupB',out_prefix)


    def analysis_1(self):
        merge_cluster={'GroupA':['0','1'],'GroupB':['2','3','4','5'],'':['6','7']}
        select_ct="LSECs"
        sc_data_key='LCA_humanCD45neg'
        out_prefix=f'{self.out_dir}/{sc_data_key}.{select_ct}'
    ...
    def analysis_3(self):
        merge_cluster={'GroupA':['1','3','4','5','8'],'GroupB':['2','10','11','0','7'],'':['6','9','12']}
